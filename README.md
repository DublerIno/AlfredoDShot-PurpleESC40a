...
Use **one** GPIO per ESC. GPIO 4, 5, 6 and 7 on the ESP32-S3 are plain I/O with
no boot-strapping duty, so they are a good pick.

```
  ESP32-S3                                          AM32 ESC
                          3V3
                           |
                          [ ] 1 kΩ            <-- required, see below
                           |
  GPIO 4  ----[ 33 Ω ]-----+------------------------ signal
                                   (optional)
  GND  -------------------------------------------- ground
...
Repeat for GPIO 5, 6, 7 → ESC 2, 3, 4. Each ESC needs its own pull-up.
...
| **33–100 Ω series resistor** | Optional. Damps ringing on longer signal wires and limits current if both ends ever drive at once. Put it at the ESP end. |
